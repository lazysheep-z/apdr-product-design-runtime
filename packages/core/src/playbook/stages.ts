...
export const STAGE_PLAYBOOKS: Record<PipelineStage, StagePlaybook> = {
  intake: {
    ...
    steps: [
      ...
      "(Required) Run skills/decision-recorder to record key product and market decisions as ADR.",
    ],
  ...
  code_intelligence: {
    ...
    skillPath: `${BASE}/code-intelligence/SKILL.md`,
    outputArtifact: "CodeIntelligenceReport",
    humanGate: "optional",
    upstreamArtifacts: ["CodeBundle"],
    externalSkills: [
      {
        id: "codegraph",
        vendorPath: "https://github.com/colbymchenry/codegraph",
      },
    ],
    objective:
      "Automatically index generated frontend code with CodeGraph and produce a structured intelligence report for downstream handoff.",
    steps: [
      "Read `skills/code-intelligence/SKILL.md`.",
      "`get_latest_artifact` CodeBundle to resolve `rootPath`.",
      "Call repo MCP `build_code_intelligence_report` — runs CodeGraph init + explore (fully automated).",
      "`write_artifact` CodeIntelligenceReport with routes, components, entry points.",
      "Auto-approve (optional gate) → `approve_latest_artifact` → `advance_pipeline`.",
    ],
    userTriggers: ["代码图谱", "codegraph", "索引代码", "code intelligence"],
    doneWhen: "CodeIntelligenceReport saved; CodeGraph index exists under app/.codegraph/.",
  },
  handoff: {
    stage: "handoff",
    ...
    humanGate: "optional",
    upstreamArtifacts: ["CodeBundle", "CodeIntelligenceReport"],
    externalSkills: [],
    objective: "Generate component docs, build instructions, API contracts, deployment guide.",
    steps: [
      "Read skills/handoff/SKILL.md.",
      "get_latest_artifact CodeBundle, CodeIntelligenceReport, UIDesignSpec, PRD.",
      "Use CodeIntelligenceReport routes/components as ground truth for route map and component list.",
  ...
  quality_eval: {
    ...
    humanGate: "optional",
    upstreamArtifacts: ["HandoffDoc", "CodeIntelligenceReport"],
    externalSkills: [],
    ...
    steps: [
      ...
      "Cross-check PRD features against CodeIntelligenceReport routes/components.",
      ...
      "(Required) Verify ADR coverage: check that all design-critical stages have decision records.",
    ],
